...
@implementation ChicagoAnimationView
...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        [self createChicagoAnimationView];
    }
    return self;
}

-(void)createChicagoAnimationView {
    
    
    CGFloat const h = self.frame.size.height;
    CGFloat const w = self.frame.size.width;
    
    UIView *bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, h*.80, w, h *.2)];
    ...
    CGPathAddLineToPoint(chicago, nil, w*.47, h*.74);
